package _07_sleepy_head;
//    Copyright (c) The League of Amazing Programmers 2013-2017

import java.io.PrintStream;

//    Level 0

import javax.swing.JOptionPane;

public class SleepyHead {

	private static final int TRUE = 0;
	private static final int FALSE =0;

	public static void main(String[] args) {

		boolean isWeekday, isVacation;

		/* You MUST use the above boolean variables in your code */

		/*
		 * Ask the user for these values using a confirm dialog like the one below
		 * JOptionPane.showConfirmDialog(null, "Is it a weekday?", "Sleepy Head",
		 * JOptionPane.YES_NO_OPTION);
		 */

		 int sleep = JOptionPane.showConfirmDialog(null, "Is it a weekday?", "Sleepy Head",
					JOptionPane.YES_NO_OPTION);
			 boolean shleep =Integer (sleep);
			if (sleep == TRUE) {
	JOptionPane.showMessageDialog(null, "gEt Up LaZy BoNeS!");
			}else {
				JOptionPane.showMessageDialog(null,"sleep unless ur wEiRd");
			}
			}
	

			 
		

	private static boolean Integer(int sleep) {
		// TODO Auto-generated method stub
		return false;
	}
	
	private static boolean Integer1(int weekend) {
		// TODO Auto-generated method stub
		return false;
	}

		/*
		 * Print “sleep in�? if it is a vacation or a weekend. If it’s a weekday, print
		 * “get up lazy-bones!�? If it is a weekday, and we are on vacation, print “sleep
		 * in�?.
		 */	}

